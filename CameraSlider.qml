...
Item {
  ...
  property color trackColor: bar
    ? Style.selectedFillFor(bar.foreground, Color.accent) : "#333"
  ...
  property real trackHeight: Math.max(4,
    Math.round(Style.spacing.controlHeight * 0.11))
  property real knobSize: Math.max(14,
  ...
  onValueChanged: if (!dragging && !awaitingConfirmation) liveValue = value

  function finishRequest(ok) {
    awaitingConfirmation = false
    if (!ok && !dragging) liveValue = value
  }
  ...
  readonly property real progress: Math.max(0,
    Math.min(1, (liveValue - minimum) / range))
  readonly property bool hot: mouseArea.containsMouse || dragging
